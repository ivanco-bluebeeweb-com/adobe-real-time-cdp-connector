"""Connection lifecycle for Adobe Real-Time CDP Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from adobe_real_time_cdp_client import AdobeRealTimeCDPClient
...
_SECRET = "adobe_real_time_cdp_connections"
...
@chat.function(
    "connect_adobe_real_time_cdp",
    "Connect Adobe Real-Time CDP account via credentials.",
    action_type="write",
    chain_callable=True,
    event="adobe-real-time-cdp-connector.connect_adobe_real_time_cdp",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_adobe_real_time_cdp(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Adobe Real-Time CDP Connector."""
    client = AdobeRealTimeCDPClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Adobe Real-Time CDP Account",
...
@chat.function(
    "list_connections",
    "List connected Adobe Real-Time CDP accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_adobe_real_time_cdp",
    "Disconnect Adobe Real-Time CDP account.",
    action_type="write",
    chain_callable=True,
    event="adobe-real-time-cdp-connector.disconnect_adobe_real_time_cdp",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_adobe_real_time_cdp(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)

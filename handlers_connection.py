...
async def connect_adobe_real_time_cdp_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected Adobe Real-Time CDP ({rec['label']}).")
...
async def disconnect_adobe_real_time_cdp_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected Adobe Real-Time CDP connection.")
